...
if (isset($_POST['submit'])) {
    ...
    if (!$product_no1) {
        ...
        $query = "UPDATE order_product SET product_no1= ? ,price1= ? ,quantity1= ? ,product_no2= ? ,price2= ?,quantity2= ?, product_no3= ?,price3= ?,quantity3= ? ,total=?, username= ? , phone= ? , email= ?, address =  ?  WHERE id = ? LiMIT 1";
        $stmt = mysqli_prepare($connection, $query);

        // bind parameters to the prepared statement
        mysqli_stmt_bind_param($stmt, "siisiisiiissssik", $product_no1, $price1, $quantity1, $product_no2 , $price2, $quantity2 , $product_no3 , $price3, $quantity3,$total,$username,$phone
    ,$email,$address,$id);
        ...
        if (mysqli_stmt_errno($stmt)) {
            $_SESSION['edit-order'] = "Failed to update order";
        } else {
            $_SESSION['edit-order-success'] = "Order of $username updated successfully";
        }
