...
//     die();
if (isset($_POST['submit'])) {
    $images_no1 = $_FILES['image_no1']['name'];
    $brand = filter_var($_POST['brand'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $name = filter_var($_POST['name'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $price = filter_var($_POST['price'], FILTER_SANITIZE_NUMBER_INT);
    $images_no2 = isset($_FILES['image_no2']) ? $_FILES['image_no2']['name'] : null;
    $images_no3 = isset($_FILES['image_no3']) ? $_FILES['image_no3']['name'] : null;
    $images_no4 = isset($_FILES['image_no4']) ? $_FILES['image_no4']['name'] : null;
    $images_no5 = isset($_FILES['image_no5']) ? $_FILES['image_no5']['name'] : null;
    $description_title = filter_var($_POST['description_title'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $description = filter_var($_POST['description'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $guarantee = filter_var($_POST['guarantee'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    $type = filter_var($_POST['type'], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    if (!$images_no1) {
        $_SESSION['-product'] = " please choose your Image No 1";
    } elseif (!$brand) {
        $_SESSION['-product'] = " please enter your Brand";
    } elseif (!$name) {
        $_SESSION['-product'] = " please enter your Name ";
    } elseif (!$price) {
        $_SESSION['-product'] = " please enter your Price";
    } elseif (!$images_no2) {
        $_SESSION['-product'] = " please choose your Image No 2";
    } elseif (!$images_no3) {
        $_SESSION['-product'] = "please choose your Image No 3";
    } elseif (!$images_no4) {
        $_SESSION['-product'] = "please choose your Image No 4";
    } elseif (!$images_no5) {
        $_SESSION['-product'] = "please choose your Image No 5";
    } elseif (!$description_title) {
        $_SESSION['-product'] = " please enter your Description Title";
    } elseif (!$description) {
        $_SESSION['-product'] = " please enter your Description";
    } elseif (!$guarantee) {
        $_SESSION['-product'] = " please enter your Guarantee";
    } elseif (!$type) {
        $_SESSION['-product'] = " please enter your Type";
    } elseif (!$cable) {
        $_SESSION['-product'] = " please enter your Cable";
    } elseif (!$switch) {
        $_SESSION['-product'] = " please enter your Switch Type";
    } elseif (!$size) {
        $_SESSION['-product'] = " please enter your Size";
    } elseif (!$connect_type) {
        $_SESSION['-product'] = " please enter your Connection Type";
    } elseif (!$color) {
        $_SESSION['-product'] = " please enter your Color";
    } elseif (!$led_color) {
        $_SESSION['-product'] = " please enter your Led Color";
    } elseif (!$keycap_material) {
        $_SESSION['-product'] = " please enter your KeyCap Material";
    } else {
        $time = time();
        $avatar_name = $time . $images_no1;
        $avatar_tmp_name = $_FILES['image_no1']['tmp_name'];
        $avatar_destination_path = '../images/' . $avatar_name;
        ...
        $extension = end($extension);
        if (in_array($extension, $allowed_files)) {
            if ($_FILES['image_no1']['size'] < 100000) {
                move_uploaded_file($avatar_tmp_name, $avatar_destination_path);
            } else {
                $_SESSION['add-product'] = 'File size too big. Should be less than 1mb.';
            }
        ...
        mysqli_stmt_bind_param($stmt, "sssissssssssssssssi", $images_no1, $brand, $name, $price, $images_no2, $images_no3, $images_no4, $images_no5, $description_title, $description, $guarantee, $type, $cable, $switch, $size, $connect_type, $color, $led_color, $keycap_material, $id);
        mysqli_stmt_execute($stmt);

        if (mysqli_error($connection)) {
            $_SESSION['edit-product'] = "Couldn't update product";
        } else {
            $_SESSION['edit-success'] = "Product $name updated successfully";
        }
    }
}
header('location:' . ROOT_URL . 'admin/manage-product.php');
