...
if (isset($_POST['submit'])) {
    ...
    if (!$username) {
        ...
        if ($_SESSION['add-checkout']) {

            $_SESSION['add-checkout-data'] = $_POST;
        ...
        foreach ($_SESSION['cart'] as $product) {
            ...
            $thumbnail = mysqli_real_escape_string($connection, $product[0]);
            $product_brand = mysqli_real_escape_string($connection, $product[4]);
            $price = floatval($product[2]);
            $quantity = intval($product[5]);

            $insert_details_query = "INSERT INTO order_details (order_id,thumbnail, name,brand, price, quantity)
            VALUES ($order_id,'$thumbnail' ,'$product_name', '$product_brand', $price, $quantity)";
        ...
        if (!mysqli_error($connection)) {
            ...
            unset($_SESSION['cart']);
            header('Location: ' . ROOT_URL . 'admin/checkout-success.php');
            die();
        } else {
